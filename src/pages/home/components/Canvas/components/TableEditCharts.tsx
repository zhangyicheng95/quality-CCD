  ...
  useEffect(() => {
    if (!!dataValue && !!localStorage.getItem('parentOrigin')) {
      ...
      window?.parent?.postMessage?.(
        { type: 'readFile', name: 'tableEditReadFile' },
      ...
      <div className="edit-charts-body">
        <Form form={form} component={false}>
          {(dataSource || [])?.map((item: any, index: number) => {
            return (
              <div className="flex-box table-edit-item-box" key={`table-edit-item-box-${index}`}>
                {(item || [])?.map((itemSec: string, indexSec: number) => {
                  return (
                    <div
                      className="flex-box-center table-edit-item-box-td"
                      key={`table-edit-item-box-td-${indexSec}`}
                    >
                      {!!itemSec ? (
                        [0, 1].includes(index) ||
                        (item?.length > 5 && [0, 1].includes(indexSec)) ? (
                          <div className="table-edit-item-box-td-title">{itemSec}</div>
