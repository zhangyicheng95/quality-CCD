...
import { downFileFun, getNewPoint, guid, hexToRGB, rotatePoint, twoPointDistance } from '@/utils/utils';
...
const MarkCanvas: React.FC<Props> = (props: any) => {
  ...
  const { platFormValue, localPath, zoom, widget, ifFetch, fetchType, xName, fontSize, inHome } =
    data;

  const { options } = widget;
  const [{ color }, open] = useEyeDropper('rgb(0, 0, 0)');
  ...
  const initDom = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      img.current.src = `${localPath?.indexOf('http') === 0
        ? localPath
        : `${BASE_IP}file${localPath?.indexOf('\\') === 0 || localPath?.indexOf('/') === 0 ? '' : '\\'
        }${localPath}`
    ...
    img.current.onload = (res: any) => {
      ...
      const scale = !!zoom
        ? zoom
        : dom?.current?.clientWidth / dom?.current?.clientHeight > width / height
      ...
      gMap.current.events.on('drawDone', (type: any, data: any) => {
        ...
        // 判断有没有画出图形之外
        if (type === 'RECT') {
          if (data.width > width && data.height > height / 2) {
            data.x = 0;
            data.y = 0;
            data.width = width;
            data.height = height;
          ...
          if (Math.min(data.x, data.y) < 0 || data.x > width || data.y > height) {
            message.warning('标注位置 不能超出图片范围！');
            return;
          }
        }
        ...
        if (type === 'MARKER') {
          ...
          const circleFeature = new AILabel.Feature.Circle(
            ...
            Object.assign(
              ...
              ['DOUBLE_CIRCLE', 'double_circle'].includes(btn)
                ? { type: 'DOUBLE_CIRCLE', child: id + 100 }
                : {},
            ), // props
            drawingStyle.current, // style
          );
          ...
          addFeatureText(
            { x: data.cx - data.r, y: data.cy - data.r },
            relatedTextId,
            inHome ? '' : 'label',
          );
        ...
        if (
          !ctrlDown.current &&
          ['LINE', 'DOUBLE_CIRCLE', 'CIRCLE', 'RECT', 'POINT', 'AXIS'].includes(btn)
        ) {
          setMode('');
        }
      });
      ...
      gMap.current.events.on('featureUpdated', (feature: any, shape: any) => {
        ...
        if (props.type === 'DOUBLE_CIRCLE') {
          const feature2 =
            gFirstFeatureLayer.current.getFeatureById(id + 100) ||
            gFirstFeatureLayer.current.getFeatureById(id - 100);
          const shape2 = {
            ...shape,
            r: feature2?.shape?.r,
  ...
  useEffect(() => {
    // initDom();
    // initColorSelector();
    // window.addEventListener('mousemove', initColorSelector);
    // dom.current.addEventListener('mousedown', onMouseDown);

    return () => {
      window.removeEventListener('keydown', onKeyDown);
      window.removeEventListener('keyup', onKeyup);
      window.removeEventListener('resize', () => gMap.current && gMap.current.resize());
      // window.removeEventListener('mousemove', initColorSelector);
      destroy();
    };
  }, []);
  // 监听键盘按下
  ...
  const addFeature = (type: any, id: any, shape: any, props: any, style: any) => {
    if (type === 'LINE') {
      const range = style.direction || 0;
      const gFirstFeatureLine = new AILabel.Feature.Line(id, shape, props, style);
      ...
      const polylineFeature = new AILabel.Feature.Polyline(id, shape, props, style);
      gFirstFeatureLayer.current.addFeature(polylineFeature);
    } else if (type === 'RECT') {
      const gFirstFeatureRect = new AILabel.Feature.Rect(id, shape, props, style);
      ...
      const gFirstFeaturePolygon = new AILabel.Feature.Polygon(id, shape, props, style);
      gFirstFeatureLayer.current.addFeature(gFirstFeaturePolygon);
    } else if (type === 'CIRCLE') {
      const gFirstFeatureCircle = new AILabel.Feature.Circle(id, shape, props, style);
      ...
      const polylineFeature = new AILabel.Feature.Point(id, shape, props, style);
      gFirstFeatureLayer.current.addFeature(polylineFeature);
      addFeatureText(shape, props.textId, props.label);
    } else {
      // const scale = gMap.current.getScale();
            ...
            props: Object.assign(
              ...
              props,
              {
                initParams: {
                  ...
                  ...(!!value?.[id]?.rotation
                    ? {}
                    : { rotation: { value: style?.direction || 0 } }),
                },
              },
  ...
  return (
    <div className={`${styles.markCanvas} flex-box`} ref={markRef}
    // onClick={() => { initColorSelector() }}
    >
      ...
      <div className="canvas-body flex-box-start">
        <div className="btn-box">
            ...
            <ClearOutlined
              onClick={() => {
                ...
                (feat || []).forEach?.((item: any) => {
                  ...
                  if (['DOUBLE_CIRCLE'].includes(props.type)) {
                    gFirstFeatureLayer.current.removeFeatureById(id + 100);
                  } else if (['AXIS'].includes(props.type)) {
                    gFirstFeatureLayer.current.removeFeatureById(id + 100);
                    gFirstFeatureLayer.current.removeFeatureById(id + 200);
                    gFirstTextLayer.current.removeTextById(props.textXId);
                    gFirstTextLayer.current.removeTextById(props.textYId);
                  }
                });
          ...
          <div className="bottom background-ubv" style={{ marginBottom: 0 }}>
            <PlusCircleOutlined className={`img-icon flex-box-center`} onClick={() => zoomIn()} />
            <MinusCircleOutlined className={`img-icon flex-box-center`} onClick={() => zoomOut()} />
        ...
        <Spin spinning={loading} tip="Loading...">
          <div className="rule-box background-ubv">
            ...
            <div className="canvas-box" id={CONTAINER_ID} ref={dom} />
          </div>
        </Spin>
                  ...
                  <Form form={form} scrollToFirstError>
                    <Form.Item
                      ...
                      initialValue={{
                        ...
                        y: {
                          alias: '原点y',
                          value: feature?.shape?.y + feature?.shape?.height / 2,
                        },
                        // ...feature?.props?.initParams?.roi?.realValue,
                    ...
                    <Form.Item
                      ...
                      initialValue={
                        featureList?.[selectedFeature]
                          ? featureList?.[selectedFeature]?.['option_type']?.value
                          : undefined
                      }
                    ...
                    {_.isEmpty(selectedOptionType)
                      ? !!featureList?.[selectedFeature]
                        ? Object.entries(featureList?.[selectedFeature])?.map?.((item: any) => {
                          if (
                            ...
                            const feature =
                              gFirstFeatureLayer.current.getFeatureById(selectedFeature);
                            if (!feature) return;
                            const { type } = feature;
                            let value = {};
                              ...
                              if (type === 'RECT') {
                                ...
                                value = {
                                  x1: {
                                    alias: '起点x',
                                    value:
                                      item[1]?.realValue?.x1?.value ||
                                      item[1]?.realValue?.start?.value?.x,
                                  },
                                  y1: {
                                    alias: '起点y',
                                    value:
                                      item[1]?.realValue?.y1?.value ||
                                      item[1]?.realValue?.start?.value?.y,
                                  },
                                  x2: {
                                    alias: '终点x',
                                    value:
                                      item[1]?.realValue?.x2?.value ||
                                      item[1]?.realValue?.end?.value?.x,
                                  },
                                  y2: {
                                    alias: '终点y',
                                    value:
                                      item[1]?.realValue?.y2?.value ||
                                      item[1]?.realValue?.end?.value?.y,
                                  },
                                };
                              } else if (type === 'CIRCLE') {
                                // 圆
                                ...
                                if (feature?.props?.type === 'DOUBLE_CIRCLE') {
                                  const feature2 =
                                    gFirstFeatureLayer.current.getFeatureById(
                                      feature?.id + 100,
                                    ) ||
                                    gFirstFeatureLayer.current.getFeatureById(feature?.id - 100);
                                  value = {
                                    ...value,
                                    r2: {
                                      alias: 'r2',
                                ...
                                value = Object.entries(_.omit(item[1], 'value')).reduce(
                                  (pre: any, cen: any) => {
                                    return Object.assign({}, pre, {
                                      [cen[0]]: {
                                        alias: cen[0],
                      ...
                      : Object.entries(selectedOptionType)?.map?.((item: any) => {
                        if (item[0] === 'roi') {
                          let value = {};
                          if (!_.isEmpty(item[1])) {
                            if (_.isNumber(item[1]?.x) && !!item[1]?.width) {
                              // 矩形
                              value = {
                                x: {
                                  alias: 'cx',
                      ...
                      .then((values) => {
                        const value = Object.entries(values).reduce((pre: any, cen: any) => {
                          ...
                          const item = Object.assign(
                            {},
                            selectedOptionType[key[0]],
                            key[0] === 'option_type' && _.isObject(cen[1])
                              ? cen[1]
                              : { value: cen[1] },
                          );
                          return Object.assign({}, pre, {
                        ...
                        if (feature?.props?.type === 'AXIS') {
                          const feature1 = gFirstFeatureLayer.current.getFeatureById(
                            feature.id + 100,
                          );
                          const feature2 = gFirstFeatureLayer.current.getFeatureById(
                            feature.id + 200,
                          );
                          const targetXText = gFirstTextLayer.current.getTextById(
                            feature?.props?.textXId,
                          );
                          const targetYText = gFirstTextLayer.current.getTextById(
                            feature?.props?.textYId,
                          );
                          const center = {
                            x:
                              value?.['roi']?.value?.x?.value ||
                              feature?.shape?.x + feature?.shape?.width / 2,
                            y:
                              value?.['roi']?.value?.y?.value ||
                              feature?.shape?.y + feature?.shape?.height / 2,
                            xLength:
                              value?.['roi']?.value?.xLength?.value || feature?.shape?.width / 2,
                            yLength:
                              value?.['roi']?.value?.yLength?.value || feature?.shape?.height / 2,
                          };
                          ...
                          const shape = {
                            x:
                              value?.['roi']?.value?.x?.value -
                              value?.['roi']?.value?.xLength?.value / 2,
                            y:
                              value?.['roi']?.value?.y?.value -
                              value?.['roi']?.value?.yLength?.value / 2,
                            width: value?.['roi']?.value?.xLength?.value,
                            height: value?.['roi']?.value?.yLength?.value,
                          };
                          let line1: any = {
                            start: {
                              x: shape.x + (shape.width * 3) / 8,
                              y: shape.y + shape.height / 2,
                            },
                            end: {
                              x: shape.x + (shape.width * 5) / 8,
                              y: shape.y + shape.height / 2,
                            },
                          },
                            line2: any = {
                              start: {
                                x: shape.x + shape.width / 2,
                                y: shape.y + (shape.height * 3) / 8,
                              },
                              end: {
                                x: shape.x + shape.width / 2,
                                y: shape.y + (shape.height * 5) / 8,
                              },
                            };
                            ...
                            [selectedFeature]: {
                              roi: {
                                value: {
                                  ...
                                  xLength: {
                                    alias: 'xLength',
                                    value: twoPointDistance(line1.start, line1.end),
                                  },
                                  yLength: {
                                    alias: 'yLength',
                                    value: twoPointDistance(line2.start, line2.end),
                                  },
                          ...
                          if (value?.['option_type']?.value) {
                            // 更新text
                            const targetText = gFirstTextLayer.current.getTextById(
                              feature?.props?.textId,
                            );
                            if (targetText && !inHome) {
                              ...
                              addFeature(
                                ...
                                id,
                                shape,
                                { ...props, label: inHome ? '' : value?.['option_type']?.value },
                                style,
                              );
                            }
                          }
                          ...
                          if (value?.['roi']?.value?.cx && value?.['roi']?.value?.r) {
                            ...
                            if (val.r2) {
                              // 圆环
                              const feature2 =
                                gFirstFeatureLayer.current.getFeatureById(selectedFeature + 100) ||
                                gFirstFeatureLayer.current.getFeatureById(selectedFeature - 100);
                              const shape2 = {
                                cx: val?.cx?.value,
                                cy: val?.cy?.value,
                          ...
                          const result = {
                            ...
                            [selectedFeature]:
                              !_.isEmpty(selectedOptionType) ||
                                !_.isEmpty(featureList?.[selectedFeature])
                                ? Object.entries(
                                  !_.isEmpty(selectedOptionType)
                                    ? selectedOptionType
                                    : featureList?.[selectedFeature],
                                    ...
                                    if (cen[0] === 'roi') {
                                      ...
                                      if (type === 'RECT') {
                                        ...
                                        val = {
                                          ...
                                          x: {
                                            ...val?.x,
                                            value: val?.x?.value - val?.width?.value / 2,
                                          },
                                          y: {
                                            ...val?.y,
                                            value: val?.y?.value - val?.height?.value / 2,
                                          },
                                        };
                                        ...
                                        const shape = Object.entries(val).reduce(
                                          (pre: any, cen: any) => {
                                            return Object.assign({}, pre, {
                                              [cen[0]]: cen[1]?.value,
                                            });
                                        ...
                                        if (
                                          ...
                                          // 删除delete-icon
                                          gMap.current.markerLayer.removeMarkerById(
                                            feature.props.deleteMarkerId,
                                          );
                                          // 先删除direction-icon
                                          gMap.current.markerLayer.removeMarkerById(
                                            feature.props.directionMarkerId,
                                          );
                                          // 后添加direction-icon
                                          const gFirstMarker = new AILabel.Marker(
