...
import { ROTATE_SVG, ROTATE_SVG_ACTIVE, ROTATE_CURSOR, COPY_SVG, DEL_SVG, COPY_SVG_ACTIVE, DEL_SVG_ACTIVE } from '@/common/constants/globalConstants';
...
const renderTBIcon = (ctx: any, left: any, top: any, styleOverride: any, fabricObject: any) => {
...
const handleCopyObject = async (eventData: any, transform: any) => {
...
const handleDelObject = (eventData: any, transform: any) => {
  const target = transform.target;
  const canvas = target.canvas;
...
export const handleMouseOverCorner = (corner: any, target: any) => {
  if (corner === 'mtr') {
...
export const handleMouseOutCorner = (target: any) => {
  if (!target) return;
