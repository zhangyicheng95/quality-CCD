...
const changeLineEnd = (eventData: any, transform: any, x: any, y: any) => {
  const { target } = transform;
  target.set({ x2: x, y2: y });
  return true;
}

const changeLineStart = (eventData: any, transform: any, x: any, y: any) => {
  const { target } = transform;
  target.set({ x1: x, y1: y });
  return true;
}

const linePositionHandler = (x: any, y: any) => {
