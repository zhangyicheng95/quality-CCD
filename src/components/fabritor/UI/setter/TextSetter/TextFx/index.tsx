...
import { drawTextPath, getPathOffset, removeTextPath } from '@/components/fabritor/editor/objects/textbox';
import { loadImageDom } from '@/components/fabritor/editor/objects/image';
...
export default function TextFx() {
  ...
  const handleTextPattern = async (pattern: any) => {
    if (!object) return;
  ...
  const handleStroke = (_stroke: any) => {
  ...
  const handleFxValueChange = async (values: any) => {
