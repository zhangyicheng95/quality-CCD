    ...
    img.onload = (res: any) => {
        ...
        if (type === 'MARKER') {
          ...
          const circleFeature = new AILabel.Feature.Circle(
            ...
            Object.assign(
              ...
              btn === 'DOUBLE_CIRCLE' ? { type: btn, child: id + 100 } : {},
            ), // props
            drawingStyle, // style
          );
      ...
      gMap.events.on('featureUpdated', (feature: any, shape: any) => {
        ...
        if (props.type === 'DOUBLE_CIRCLE') {
          const feature2 =
            gFirstFeatureLayer.getFeatureById(id + 100) ||
            gFirstFeatureLayer.getFeatureById(id - 100);
          const shape2 = {
            ...shape,
            r: feature2?.shape?.r,
          ...
          <div className="top background-ubv">
            ...
            <div
              className={`img-icon flex-box-center ${
                selectedBtn === 'DOUBLE_CIRCLE' ? 'selected' : ''
              }`}
              onClick={() => {
            ...
            <div
              className={`img-icon flex-box-center ${selectedBtn === 'CIRCLE' ? 'selected' : ''}`}
              onClick={() => {
                setMode('CIRCLE');
              }}
            >
                              ...
                              if (_.isObject(item[1]?.realValue) && !_.isEmpty(item[1].realValue)) {
                                if (!!item[1]?.realValue?.x && !!item[1]?.realValue?.width) {
                                  ...
                                  if (feature?.props?.type === 'DOUBLE_CIRCLE') {
                                    const feature2 =
                                      gFirstFeatureLayer.getFeatureById(feature?.id + 100) ||
                                      gFirstFeatureLayer.getFeatureById(feature?.id - 100);
                                    value = {
                                      ...value,
                                      r2: {
                      ...
                      .then((values) => {
                        ...
                        if (feature?.props?.type === 'AXIS') {
                          ...
                          const center = {
                            ...
                            yLength:
                              value?.['roi']?.value?.yLength?.value || feature?.shape?.height,
                          };
                          ...
                          if (range !== feature?.style?.direction) {
                            ...
                            if (!!feature?.style?.direction) {
                              line1 = {
                                start: rotatePoint(
                                  feature1?.shape?.start,
                                  center,
                                  -feature?.style?.direction,
                                ),
                                end: rotatePoint(
                                  feature1?.shape?.end,
                                  center,
                          ...
                          const shape = {
                            x:
                              center.x -
                              Math.max(
                                Math.abs(line1.end.x - line1.start.x),
                                Math.abs(line2.end.x - line2.start.x),
                              ) /
                                2,
                            y:
                              center.y -
                              Math.max(
                                Math.abs(line1.end.y - line1.start.y),
                                Math.abs(line2.end.y - line2.start.y),
                              ) /
                                2,
                            width: Math.max(
                              Math.abs(line1.end.x - line1.start.x),
                          ...
                          if (value?.['option_type']?.value) {
                            ...
                            if (targetText) {
                              ...
                              addFeature(
                                ...
                                id,
                                shape,
                                { ...props, label: value?.['option_type']?.value },
                                style,
                              );
                            }
                          }
                          ...
                          if (value?.['roi']?.value?.cx && value?.['roi']?.value?.r) {
                            ...
                            if (val.r2) {
                              // 圆环
                              const feature2 =
                                gFirstFeatureLayer.getFeatureById(selectedFeature + 100) ||
                                gFirstFeatureLayer.getFeatureById(selectedFeature - 100);
                              const shape2 = {
                                cx: val?.cx?.value,
                                cy: val?.cy?.value,
                              ...
                              !_.isEmpty(featureList[selectedFeature])
                                ? Object.entries(
                                    ...
                                    (pre: any, cen: any) => {
                                      if (cen[0] === 'roi') {
                                        const feature =
                                          gFirstFeatureLayer.getFeatureById(selectedFeature);
                                        const { type } = feature;
                                        ...
                                        if (type === 'RECT') {
                                          ...
                                          if (
                                            ...
                                            feature.style['direction'] = range;
                                            feature.updateShape(shape);
                                            const targetText = gFirstTextLayer.getTextById(
                                              feature?.props?.textId,
                                            );
                                            targetText?.updatePosition({ x: shape.x, y: shape.y });
                                            // 删除delete-icon
                                            gMap.markerLayer.removeMarkerById(
